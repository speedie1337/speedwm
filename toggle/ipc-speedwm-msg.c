...
const char *DEFAULT_SOCKET_PATH = "/tmp/speedwm.sock";
...
typedef enum IPCMessageType {
  ...
  IPC_TYPE_GET_SPEEDWM_CLIENT = 4,
  IPC_TYPE_SUBSCRIBE = 5,
  IPC_TYPE_EVENT = 6
} IPCMessageType;

// Every IPC message must begin with this
typedef struct speedwm_ipc_header {
...
send_message(IPCMessageType msg_type, uint32_t msg_size, uint8_t *msg)
{
  ...
  size_t header_size = sizeof(speedwm_ipc_header_t);
...
get_speedwm_client(Window win)
{
  ...
  send_message(IPC_TYPE_GET_SPEEDWM_CLIENT, msg_size, (uint8_t *)msg);
...
print_usage(const char *name)
{
  ...
  puts("");
  puts("  get_speedwm_client <window_id>  Get speedwm client proprties");
  puts("");
...
main(int argc, char *argv[])
{
  ...
  if (strcmp(argv[i], "help") == 0)
    ...
    get_layouts();
  } else if (strcmp(argv[i], "get_speedwm_client") == 0) {
    if (++i < argc) {
      if (is_unsigned_int(argv[i])) {
        Window win = atol(argv[i]);
        get_speedwm_client(win);
      } else
